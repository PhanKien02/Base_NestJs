...
import { LoginDto, PayLoadToken } from '@/models/dto/login.dto';
    ...
    async login(payLoad: LoginDto) {
        ...
        return {
            ...
            expires: expires.setSeconds(
                expires.getSeconds() + parseInt(configuration().accessToken.expiresIn),
            ),
        };
    }
    async validateUser(
        payLoad: LoginDto,
    ): Promise<Omit<IUser, 'password' | 'activeKey' | 'resetKey'>> {
